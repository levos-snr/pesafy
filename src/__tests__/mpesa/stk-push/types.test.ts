...
const successWrapper: StkPushCallback = {
  Body: { stkCallback: successCallback },
}
const failureWrapper: StkPushCallback = {
  Body: { stkCallback: failureCallback },
}
...
describe('getCallbackValue', () => {
  ...
  it('extracts MpesaReceiptNumber from a successful callback', () => {
    expect(getCallbackValue(successWrapper, 'MpesaReceiptNumber')).toBe(
      'QKA81LK5CY',
    )
  })
