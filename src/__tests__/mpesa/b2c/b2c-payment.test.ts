...
describe("initiateB2CPayment — validation", () => {
  ...
  it.each([
    ...
    ["invalid commandId", { ...VALID, commandId: "InvalidCommand" as B2CCommandID }],
    ...
    await expect(initiateB2CPayment(BASE_URL, "tok", "cred", "testapi", req)).rejects.toMatchObject(
...
describe("initiateB2CPayment — payload", () => {
  ...
  it.concurrent("sends SenderIdentifierType '4' by default", async () => {
    ...
    await initiateB2CPayment(BASE_URL, "tok", "cred", "testapi", VALID);
    const body = vi.mocked(httpRequest).mock.calls.at(-1)?.[1]?.body as Record<string, unknown>;
