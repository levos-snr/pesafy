  ...
  Result: {
    ...
    ResultParameters: {
      ResultParameter: [
        {
          Key: 'AccountBalance',
          Value: 'Working Account|KES|500.00|Utility Account|KES|0.00',
        },
      ],
    },
  },
...
describe('parseAccountBalance', () => {
  ...
  it('parses multiple accounts from a single pipe-delimited string', () => {
    const parsed = parseAccountBalance(
      'Working Account|KES|500.00|Utility Account|KES|0.00',
    )
...
describe('getAccountBalanceParam', () => {
  ...
  it('returns undefined for a missing key', () => {
    expect(
      getAccountBalanceParam(SUCCESS_RESULT, 'NonExistentKey'),
    ).toBeUndefined()
  })

  it('returns undefined when ResultParameters are absent', () => {
    expect(
      getAccountBalanceParam(FAILURE_RESULT, 'AccountBalance'),
    ).toBeUndefined()
  })
})
