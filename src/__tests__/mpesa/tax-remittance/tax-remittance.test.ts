...
import { KRA_SHORTCODE, remitTax, TAX_COMMAND_ID } from "../../../mpesa/tax-remittance/remit-tax";
...
describe("remitTax — validation", () => {
  ...
  ])("throws VALIDATION_ERROR when %s", async (_label: string, req: TaxRemittanceRequest) => {
    await expect(remitTax(BASE_URL, "tok", "cred", "testapi", req)).rejects.toMatchObject({
...
describe("remitTax — payload", () => {
  ...
  it("allows overriding PartyB when provided", async () => {
    mockHttp.mockResolvedValueOnce({ data: OK_RESP, status: 200, headers: {} });
    await remitTax(BASE_URL, "tok", "cred", "testapi", { ...VALID, partyB: "999999" });
    const body = mockHttp.mock.calls[0]![1].body as Record<string, unknown>;
