...
describe("sendBulkInvoices", () => {
  ...
  it("throws VALIDATION_ERROR for an empty invoices array", async () => {
    await expect(sendBulkInvoices(BASE_URL, "tok", { invoices: [] })).rejects.toMatchObject({
      code: "VALIDATION_ERROR",
    });
    expect(mockHttp).not.toHaveBeenCalled();
  });
...
describe("cancelInvoice", () => {
  ...
  it("throws VALIDATION_ERROR when externalReference is empty", async () => {
    await expect(cancelInvoice(BASE_URL, "tok", { externalReference: "" })).rejects.toMatchObject({
      code: "VALIDATION_ERROR",
    });
    expect(mockHttp).not.toHaveBeenCalled();
  });
