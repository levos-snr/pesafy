...
vi.mock('../../mpesa/account-balance/query', () => ({
...
describe('Mpesa constructor', () => {
  ...
  it('throws INVALID_CREDENTIALS when consumerKey is missing', () => {
    expect(() => new Mpesa({ ...VALID_CONFIG, consumerKey: '' })).toThrow(
      PesafyError,
    )
  })

  it('throws INVALID_CREDENTIALS when consumerSecret is missing', () => {
    expect(() => new Mpesa({ ...VALID_CONFIG, consumerSecret: '' })).toThrow(
      PesafyError,
    )
  })
...
describe('Mpesa.stkPush', () => {
  ...
  it('throws VALIDATION_ERROR when shortCode not configured', async () => {
    const mpesa = new Mpesa({
      ...VALID_CONFIG,
      lipaNaMpesaShortCode: undefined,
    })
    await expect(
      mpesa.stkPush({
        amount: 100,
