...
describe('httpRequest — success', () => {
  it('returns parsed JSON for a 200 response', async () => {
    fetchSpy.mockResolvedValueOnce(
      makeFetchResponse({ access_token: 'tok123' }),
    )
...
describe('httpRequest — retries', () => {
  ...
  it.each([429, 500, 502, 503, 504])(
    'retries on HTTP %i (transient)',
    async (status) => {
      // Fail once with a transient status, then succeed
      fetchSpy
        .mockResolvedValueOnce(
          makeFetchResponse({ error: 'transient' }, status),
        )
  ...
  it.each([400, 401, 403, 404, 422])(
    ...
    async (status) => {
      fetchSpy.mockResolvedValueOnce(
        makeFetchResponse({ error: 'bad request' }, status),
      )
...
describe('httpRequest — network errors', () => {
  ...
  it('retries on network failure and then succeeds', async () => {
    ...
    const res = await httpRequest<{ recovered: boolean }>(
      'https://example.com',
      {
        method: 'GET',
        retries: 1,
...
describe('httpRequest — error body parsing', () => {
  it.concurrent(
    'extracts errorMessage from Daraja error envelope',
    async () => {
      const localFetch = vi
        .fn()
        .mockResolvedValueOnce(
          makeFetchResponse({ errorMessage: 'Invalid Access Token' }, 400),
        )
  ...
  it.concurrent(
    'extracts ResponseDescription when errorMessage absent',
    async () => {
      const localFetch = vi
        .fn()
        .mockResolvedValueOnce(
          makeFetchResponse({ ResponseDescription: 'Wrong credentials' }, 401),
        )
      vi.stubGlobal('fetch', localFetch)
  ...
  it.concurrent('throws a PesafyError (not a plain Error)', async () => {
    const localFetch = vi
      .fn()
      .mockResolvedValueOnce(makeFetchResponse({ error: 'gone' }, 500))
    vi.stubGlobal('fetch', localFetch)
