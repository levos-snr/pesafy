...
beforeAll(() => {
  expect.extend({
        ...
        message: () =>
          pass
            ...
            : `Expected a PesafyError with code "${code}", got: ${
                received instanceof PesafyError
                  ? `PesafyError(${received.code})`
                  : String(received)
              }`,
      }
    },
    ...
    toBeRetryable(received: unknown) {
      const pass =
        received instanceof PesafyError && received.retryable === true
      return {
        pass,
        message: () =>
          pass
...
describe('PesafyError', () => {
  ...
  it("has name 'PesafyError'", () => {
    expect(new PesafyError({ code: 'API_ERROR', message: 'x' }).name).toBe(
      'PesafyError',
    )
  })
  ...
  it('isValidation is true for VALIDATION_ERROR', () => {
    expect(
      new PesafyError({ code: 'VALIDATION_ERROR', message: 'v' }).isValidation,
    ).toBe(true)
  })

  it('isAuth is true for AUTH_FAILED', () => {
    expect(new PesafyError({ code: 'AUTH_FAILED', message: 'a' }).isAuth).toBe(
      true,
    )
  })
  ...
  it('isAuth is true for INVALID_CREDENTIALS', () => {
    expect(
      new PesafyError({ code: 'INVALID_CREDENTIALS', message: 'a' }).isAuth,
    ).toBe(true)
  })
...
describe('createError', () => {
  it('returns a PesafyError instance', () => {
    expect(createError({ code: 'API_ERROR', message: 'x' })).toBeInstanceOf(
      PesafyError,
    )
  })

  it('uses the custom toBeApiError matcher via factory', () => {
    expect(createError({ code: 'RATE_LIMITED', message: 'slow' })).toBeApiError(
      'RATE_LIMITED',
    )
  })
})
...
describe('isPesafyError', () => {
  it('returns true for PesafyError', () => {
    expect(
      isPesafyError(new PesafyError({ code: 'TIMEOUT', message: 't' })),
    ).toBe(true)
  })
